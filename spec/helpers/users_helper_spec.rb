...
RSpec.describe UsersHelper, type: :helper do
  ...
  context '#top_artist_attributes' do
    it "returns a string containing the desired attributes" do
      expect(UsersHelper.top_artist_attributes).to be_a String
      expect(UsersHelper.top_artist_attributes).to eq(
  ...
  context '#time_range_month' do
    it 'returns a range' do
      time = DateTime.new(2017, 3, 9)
      expect(UsersHelper.time_range_month(time)).to be_a Range
    end
  end
  ...
  context '#month_start' do
    it 'returns a DateTime object with the first day of the month' do
      time = DateTime.new(2017, 3, 9)
      expect(UsersHelper.month_start(time)).to be_a DateTime
      expect(UsersHelper.month_start(time)).to eq "Wed, 01 Mar 2017 00:00:00.000000000 +0000"
    end
  end
  ...
  context '#month_end' do
    it 'returns a DateTime object with the last day of the month' do
      time = DateTime.new(2017, 3, 9)
      expect(UsersHelper.month_end(time)).to be_a DateTime
      expect(UsersHelper.month_end(time)).to eq "Fri, 31 Mar 2017 23:59:00.000000000 +0000"
    end
  end
