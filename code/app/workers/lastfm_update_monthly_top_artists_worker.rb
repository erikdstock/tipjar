class LastfmUpdateMonthlyTopArtistsWorker
  ...
  def perform(id, month)
    ...
    if user
      # get current artists for period
      current_top_artists = user.timely_top_artists(time_range)
      ...
      # update
      user.update_top_artists_for_month(current_artists: current_top_artists,
                                        new_artists: new_top_artists,
